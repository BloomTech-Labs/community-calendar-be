...
//  Event-8
  ...
  const event9 = await prisma.createEvent({
      index: ",oktoberfest,st,joseph,or,ground,detroit,church,wil,host,stein,hold,contest,kid,are,vend,liv,mus,dant,food,drink,insid,ensembl,tour,build",
      title: "Oktoberfest at St. Joseph Oratory",
      description: "The grounds of this Detroit church will host a stein-holding contest, a kids area, vendors, live music, dancing, food and drink. Inside the church there are live musical ensembles and tours of the building",
      start: "2020-09-27T17:30:00.000Z",
      end: "2020-09-28T03:30:00.000Z",
      ticketPrice: 0,
      creator: {connect: {id: user.id}},
      eventImages: {
        create: [{
          url: "https://res.cloudinary.com/communitycalendar/image/upload/v1580752094/hmvpumh6zou94nlvpztv.jpg",
          creator: {connect: {id: user.id}},
        }]
      },
      admins: {connect: [{id: user.id}]},
      ...
      tags: {connect: [{title: "oktoberfest"}, {title: "beer"}, {title: "dancing"}]}
    }
  ) 
...
const event10 = await prisma.createEvent({
      index: "hot,dog,eat,contest,doz,coney,doesn,faz,an,am,island,challeng,aug,29,cal,nam,ten,year,competit,giv,10,minut,down,top,chil,mustard,on,win,get,weekend,trip,las,vega,two,fre,meal,champ,belt",
      title: "Hot Dog Eating Contest",
      description: "If eating a dozen or so coney dogs doesn't faze you, the annual American Coney Island coney dog eating challenge on Aug. 29 is calling your name. \n\nNow in its tenth year, the yearly competition gives eaters 10 minutes to down as many hot dogs topped with chili, mustard and onions as they can. This year's winner gets a weekend trip to Las Vegas for two, free American Coney Island meals for a year and the championship belt. ",
      start: "2020-08-29T22:00:00.000Z",
      end: "2020-08-30T00:00:00.000Z",
      ticketPrice: 0,
      creator: {connect: {id: user.id}},
      eventImages: {
        create: [{
          url: "https://res.cloudinary.com/communitycalendar/image/upload/v1580762729/gjmce6xoz2blud8atkso.jpg",
          creator: {connect: {id: user.id}},
        }]
      },
      admins: {connect: {id: user.id}},
      ...
      tags: {connect: [{title: "food"}, {title: "hotdog"}, {title: "competition"}]}
    }
  ) 
...
const event11 = await prisma.createEvent({
      index: ",walk,oceansid,enjoy,tim,outsid,oc,sid,feel,fre,bring,famy,fun,everyon,",
      title: "Walk to the Oceanside",
      description: "Come enjoy some time outside on our walk to the ocean side. Feel free to bring your family and have fun with everyone.",
      start: "2020-02-03T20:00:00.000Z",
      end: "2020-02-03T23:00:00.000Z",
      ticketPrice: 0,
      creator: {connect: {id: user.id}},
      eventImages: {
        create: [{
          url: "https://res.cloudinary.com/communitycalendar/image/upload/v1580749924/ccw8ebtvzxtoid7jqwaq.jpg",
          creator: {connect: {id: user.id}},
        }]
      },
      admins: {connect: {id: user.id}},
